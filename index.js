...
for (let key in process.env) {
  ...
  if (key.toString().indexOf("TABS") != -1) {
    ...
    total_count += Number(process.env[key]);
  }
}
...
function auto_login_read(addr, tab, no) {
  ...
  (async () => {
    ...
    function open_tab() {
      let curr_open_tab = 1;
      let id = setInterval(() => {
        tabsopen(curr_open_tab, start, 1);
        start += 1;
        ...
        const loop = async () => {
          ...
          try {
            ...
            let max_tab_num = await page.evaluate(async () => {
              let max_num = 0;
              const max = (item) => max_num < item.innerText.trim() ? item.innerText : max_num;
            ...
            if (goto && start <= max_tab_num) {
              try {
                ...
                if (++write_order >= total_count) {
                  write_order = total_count;
                  ...
                  let for_confirm = old_data.filter((item1) =>
                    !confirm_data.some((item2) =>
                      item1.transaction.toString().trim() == item2.transaction.toString().trim() &&
                      item1.block.toString().trim() == item2.block.toString().trim() &&
                      item1.from.toString().trim() == item2.from.toString().trim() &&
                      item1.to.toString().trim() == item2.to.toString().trim() &&
                  ...
                  if (flag && for_confirm.length > 0) {
                    ...
                    try {
                      for (let index = 0; index < for_confirm.length; index++) {
                        await ts_page.goto(ts_addr + for_confirm[index].transaction);
                        await ts_page.waitForSelector("div.d-flex.band-item.item-belong");

                        const get_data = await ts_page.evaluate(async () => {
                        ...
                        for_confirm[index].Bandwidth = get_data[0];
                        for_confirm[index].Energy = get_data[1];
                        for_confirm[index].status = get_data[2];
                      }
                    } catch (err) { }

                    ts_page.close();

                    let filter_data = for_confirm.filter((item1) =>
                      item1.status == "CONFIRMED"
                    );

                    if (filter_data.length > 0) {
                      filter_data.forEach((item) => {
                        confirm_data.push(item);
                      })
                      axios.post(process.env.WEBHOOK_URL, JSON.stringify(filter_data, null, 2))
                      ...
                      try {
                        fs.writeFile(
                          `./data_${confirm_data[0].coin}.txt`,
                          JSON.stringify(confirm_data, null, 2),
                          function (err, result) { }
                        );
                      } catch (err) { }
                      ...
                      fs.appendFile(
                        "./code.log",
                        `       + Input new transactions to data_${confirm_data[0].coin}.txt \n`,
                        function (err, result) { }
                      );
                      console.log(`       + Input new transactions to data_${confirm_data[0].coin}.txt`);
                    }
                    flag = true;
                  }
                }
